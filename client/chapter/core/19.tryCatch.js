...
try {
  let user = JSON.parse(data);
  // user.name; // catch 로 가서 에러가 콘솔에 찍힘
  if (!user.value) {
    // undefiend 가 뜬다 undefiend -> !false -> true
    throw new ReferenceError("해당 키값은 존재 ㄴ ");
  }
  console.log(user.value);
} catch (err) {
  // 케치가 에러를 가져와서 에러를 출력해준다.
  console.log("JSON Error : " + err.message);
} finally {
  // try 를 하든 catch 를 하든 한번은 무조건 실행
  console.log("임무완료");
}
